...
import edu.ufp.inf.sd.rabbit.advanceWars.consumer.aw_game.menus.Pause;
 ...
 * Menus are being moved to edu.ufp.inf.sd.rabbit.advanceWars.consumer.aw_game.gui.gms
 * @author SergeDavid
 * @version 0.1
 */
...
public class InputHandler implements KeyListener,MouseListener,ActionListener {
	...
	public void keyPressed(KeyEvent e) {
		...
		if (Game.GameState==Game.State.PLAYING) {
			edu.ufp.inf.sd.rabbit.advanceWars.consumer.aw_game.players.Base ply = Game.player.get(Game.btl.currentplayer);
			try {
				if (i==up) {
					String move="up;"+Game.id;
					Game.observer.channel.basicPublish("","queue_w",null,move.getBytes(StandardCharsets.UTF_8));
				}
				else if (i==down) {
					String move="down;"+Game.id;
					Game.observer.channel.basicPublish("","queue_w",null,move.getBytes(StandardCharsets.UTF_8));
				}
				else if (i==left) {
					String move="left;"+Game.id;
					Game.observer.channel.basicPublish("","queue_w",null,move.getBytes(StandardCharsets.UTF_8));
				}
				else if (i==right) {
					String move="right;"+Game.id;
					Game.observer.channel.basicPublish("","queue_w",null,move.getBytes(StandardCharsets.UTF_8));
				}
				else if (i==select) {
					String move="select;"+Game.id;
					Game.observer.channel.basicPublish("","queue_w",null,move.getBytes(StandardCharsets.UTF_8));
				}
				else if (i==cancel) {
					String move="cancel;"+Game.id;
					Game.observer.channel.basicPublish("","queue_w",null,move.getBytes(StandardCharsets.UTF_8));
				}
				else if (i==start) {
					String move="start;"+Game.id;
					Game.observer.channel.basicPublish("","queue_w",null,move.getBytes(StandardCharsets.UTF_8));
					new Pause();
				}
		...
		if (Game.GameState==Game.State.EDITOR) {
			if (i==up) {Game.edit.selecty--;if (Game.edit.selecty<0) {Game.edit.selecty++;} Game.edit.moved = true;}
				...
				new edu.ufp.inf.sd.rabbit.advanceWars.consumer.aw_game.menus.EditorMenu();
			}
		}
		...
		else if (i==dev6) {new edu.ufp.inf.sd.rabbit.advanceWars.consumer.aw_game.menus.StartMenu();}
	}
